...
contract ps{
    ...
    string a = "A";
    function changeB(string memory _B)public check{
        
        a = _B;
    }
    function changeC(string memory _B)public check {
        a = _B;
    }
    ...
    // function qotn(uint _a) public pure returns(string memory){
    //     return string(abi.encodePacked(_a/2,"2of odd " ,_a/3,"3of od " , _a/5, "5of od " ,_a/7,"7of od ");        
    // }
...
contract q4{
    uint[] fs ;
    function setfs(uint _a) public {
        fs.push(_a);
    }
    function getfs() public view returns (uint[] memory) {
        return fs;
    }
    function center() public view returns(uint[] memory) {
        // require(fs[0] > fs[fs.length]);
        uint[] memory a1= new uint[](1);
        ...
        if(fs.length%2 == 1 ){
           ...
           return a1;
            ...
            a2[0] = fs[(fs.length/2)-1];
            a2[1] = fs[(fs.length/2)];
            return a2;
            // return fs[fs.length]
        }
    }
    function oruem() public {                          
            for(uint i=0;i<fs.length-1;i++) {
                    for(uint j=i+1; j<fs.length ;j++) {
                        if(fs[i]< fs[j]) {
                         (fs[i], fs[j]) = (fs[j], fs[i]);
                }
            }
