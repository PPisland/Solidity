...
contract Q6 {
    ...
    // * 투표하는 기능 - 특정 안건에 대하여 투표하는 기능, 안건은 제목으로 검색, 이미 투표한 건에 대해서는 재투표 불가능
    function vote(string memory _title, bool _vote) external/*상속받은 애도 못하게*/ IsItUser {
    ...
    function finishPoll(string memory _title) external IsItUser {
        ...
        if(polls[_title].pros + polls[_title].cons > count*7/10 && //solidity에서는 곱하고 난 후 나눌것 왜냐하면 다른 값이 나옴 소수점 떼버리기 떄문에,
        (polls[_title].pros) *100/(polls[_title].pros + polls[_title].cons) > 66){
